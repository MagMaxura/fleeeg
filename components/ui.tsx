...
export const Icon: React.FC<{ type: string; className?: string }> = ({ type, className = "w-6 h-6" }) => {
  const icons: { [key: string]: React.ReactNode } = {
    ...
    arrowUpCircle: <path strokeLinecap="round" strokeLinejoin="round" d="M15 11.25l-3-3m0 0l-3 3m3-3v7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />,
    arrowDownCircle: <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75l3 3m0 0l3-3m-3 3v-7.5M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />,
    users: <path strokeLinecap="round" strokeLinejoin="round" d="M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m-7.5-2.962c.57-1.023 1.535-1.84 2.6-2.372a6.03 6.03 0 017.5 0c1.065.532 2.03 1.35 2.6 2.372m-10.1-2.372a6.03 6.03 0 00-7.5 0c-1.065.532-2.03 1.35-2.6 2.372M15 9.75a3 3 0 11-6 0 3 3 0 016 0z" />
  };
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ children, className, variant = 'primary', isLoading = false, size = 'default', ...props }, ref) => {
    ...
    return (
      <button ref={ref} className={`${baseClasses} ${sizeClasses[size]} ${variantClasses[variant]} ${className || ''}`} disabled={isLoading} onClick={createRipple} {...props}>
